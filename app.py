...
# ── AI SETUP WITH AUTOMATIC FALLBACK ──────────────────────────────────────────
# Groq is primary. If Groq fails → Gemini kicks in. App never breaks.
try:
...
INDEX       = "reunion_cases"
SYSTEM_MSG  = "You are REUNION, an expert AI for missing persons in India. Always respond with valid JSON only. No markdown, no explanation."
    ...
    with st.spinner("🤖 AI analysing..."):
        ...
        if groq_client:
            try:
                ...
                st.toast("⚡ Switching to backup AI...", icon="🔄")
                text = None

        # ── Fallback to Gemini ──
        if text is None and gemini_model:
            try:
                ...
                return {"raw_response": str(e), "parse_error": True}

        if text is None:
            return {"raw_response": "No AI available", "parse_error": True}
...
def ask_ai_chat(messages: list, context: str = "") -> str:
    system = ("You are REUNION, an empathetic AI agent helping reunite missing persons in India. "
              "Mention Elastic search when you find cases. Always highlight the consent wall. "
              "Be warm and helpful." + context)
    if groq_client:
        try:
            r = groq_client.chat.completions.create(
                model=GROQ_MODEL,
    ...
    if gemini_model:
        try:
            ...
            return chat.send_message(system+"\n\n"+messages[-1]["content"]).text
        except: pass
    return "AI temporarily unavailable. Please try again in a moment."

def age_progression(case):
    years=2026-case.get("year_missing",2000); age_then=case.get("age_when_missing",0); age_now=age_then+years
...
.badge-ai{background:#4285F4;color:#fff;padding:3px 10px;border-radius:99px;font-size:11px;font-weight:600}
</style>
""",unsafe_allow_html=True)

# Show which AI is active
ai_label = "Groq AI" if groq_client else "Gemini AI" if gemini_model else "No AI"
ai_color = "#f55036" if groq_client else "#4285F4"
...
<div class="reunion-header">
  <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      ...
      <span style="background:{ai_color};color:#fff;padding:3px 10px;border-radius:99px;font-size:11px;font-weight:600">{ai_label}</span>
...
with st.sidebar:
    ...
    except: st.error("❌ Elastic Offline")

    # Show AI status
    st.markdown("---")
    if groq_client:   st.success("✅ Groq AI Active")
    elif gemini_model: st.warning("⚡ Gemini AI Active (backup)")
    else:              st.error("❌ No AI Connected")
...
if page=="🏠 Home":
    ...
    for col,num,label,color in zip([c1,c2,c3,c4],[total,active,pending,reunited],
    ...
    with col1:
        ...
        for num,title,desc in [
            ...
            st.markdown(f'<div style="display:flex;gap:12px;align-items:flex-start;margin-bottom:12px"><div style="background:#e94560;color:white;width:24px;height:24px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:600;flex-shrink:0">{num}</div><div><strong style="color:#e0e0e0">{title}</strong><br><span style="color:#a0a0b0;font-size:13px">{desc}</span></div></div>',unsafe_allow_html=True)
    with col2:
        st.markdown("### 📖 Demo Story — Lakshmi Bai")
        st.markdown("""<div style="background:#1a1a2e;border:1px solid #e94560;border-radius:10px;padding:1.2rem">
  ...
  <div style="background:#2d2d44;height:1px;margin:8px 0"></div>
  <div style="font-size:12px;color:#a0a0b0;line-height:1.8">🧬 Age progression: 8 → 35 years<br>🔍 Elastic finds case instantly<br>📊 Sighting match: 90% confidence<br>🔒 Consent wall: both verify Aadhaar<br>🎉 Reunion after <strong style="color:#e94560">27 years</strong></div>
</div>""",unsafe_allow_html=True)
    st.markdown("---")
    st.markdown("### 🔐 4-Tier Access System")
    t1,t2,t3,t4=st.columns(4)
    for col,tier,title,desc,color in zip([t1,t2,t3,t4],["1","2","3","4"],
...
# ── SEARCH ─────────────────────────────────────────────────────────────────────
    ...
    with col1: query=st.text_input("Search...",placeholder="e.g. Lakshmi · birthmark · tribal girl · Kolkata")
    with col2: status_filter=st.selectbox("Filter",["All","active_search","matched_pending_consent","reunited","unidentified"])
    if query:
        with st.spinner("🔍 Searching Elastic..."): results=search_cases(query)
        if status_filter!="All": results=[r for r in results if r.get("status")==status_filter]
        if results:
            st.success(f"Found **{len(results)}** matching case(s)")
            for case in results:
                status=case.get("status","active"); emoji,color=STATUS_COLOR.get(status,("⚪","#888"))
                with st.expander(f"{emoji} **{case.get('name','Unidentified')}** — `{case.get('case_id','N/A')}` — *{status.replace('_',' ').upper()}*"):
                    ...
                    marks=case.get("distinguishing_marks","")
                    if marks: st.markdown(f"🔍 **Distinguishing Marks:** `{marks}`")
                    if case.get("sightings"): st.markdown(f"👁️ **Sightings on record:** {len(case.get('sightings',[]))}")
                    if case.get("tags"): st.markdown("**Tags:** "+" ".join([f"`{t}`" for t in case.get("tags",[])]))
        else: st.warning("No cases found. Try different search terms.")
...
# ── FILE NEW CASE ──────────────────────────────────────────────────────────────
    ...
    if aadhaar_input:
        ...
        if verified_user: st.success(f"✅ Verified: **{verified_user['name']}** ({verified_user['role'].replace('_',' ').title()})")
    ...
    if verified_user:
        with st.form("new_case"):
            ...
            col1,col2=st.columns(2)
            with col1:
                name=st.text_input("Full Name *"); age=st.number_input("Age when missing *",0,120,10)
                gender=st.selectbox("Gender *",["Female","Male","Other"]); year=st.number_input("Year missing *",1947,2026,2020)
            with col2:
                state=st.text_input("Last known state *"); district=st.text_input("Last known district (optional)")
            ...
            distinguishing=st.text_area("Distinguishing marks *",height=60)
            ...
            with col2:
                languages=st.text_input("Languages spoken (optional)"); religion=st.text_input("Religion/Community (optional)")
            st.warning("⚠️ By submitting, you confirm this is a genuine report.")
            ...
            if submitted:
                if not all([name,state,description,distinguishing,circumstances,reporter_phone]):
                    ...
                    st.markdown(f'<div style="background:#0f2a0f;border:2px solid #4caf50;border-radius:10px;padding:1.5rem;text-align:center;margin:1rem 0"><div style="font-size:13px;color:#a0a0b0">Your Case ID — save this!</div><div style="font-size:2rem;font-weight:700;color:#4caf50;font-family:monospace">{case_id}</div></div>',unsafe_allow_html=True)
                    ...
                    for i,step in enumerate(risk.get("next_steps",[]),1): st.markdown(f"{i}. {step}")
...
# ── REPORT SIGHTING ────────────────────────────────────────────────────────────
    ...
    if verified_s:
        ...
        if cases:
            ...
            if selected_case:
                status=selected_case.get("status",""); emoji,_=STATUS_COLOR.get(status,("⚪","#888"))
                ...
                if marks: st.markdown(f"🔍 **Key identifier to look for:** `{marks}`")
...
#geo_status{color:#a0c0a0;font-size:12px;margin-top:4px}</style>
...
function getLocation(){
  ...
  if(!navigator.geolocation){s.innerText='❌ Not supported.';btn.disabled=false;return;}
  navigator.geolocation.getCurrentPosition(function(pos){
    ...
    .then(function(data){
      var a=data.address||{};
      var parts=[a.suburb||a.neighbourhood||'',a.city||a.town||'',a.state||'',a.country||''].filter(function(x){return x.trim()!='';});
      var loc=parts.join(', ');out.value=loc;s.innerText='✅ '+loc;btn.innerText='✅ Detected';
      window.parent.postMessage({type:'streamlit:setComponentValue',value:loc},'*');
    }).catch(function(){out.value=lat+', '+lon;btn.innerText='✅ GPS';});
  },function(err){s.innerText='❌ Permission denied. Type manually.';btn.disabled=false;btn.innerText='📍 Try Again';},{timeout:10000});
}</script>""",height=100)
        auto_loc=st.session_state.get("detected_location","")
        with st.form("sighting_form"):
            sighting_desc=st.text_area("Describe the person you saw *",height=120,placeholder="Physical appearance, clothing, behaviour — mention any marks or scars!")
            sighting_location=st.text_input("Location *",value=auto_loc,placeholder="Auto-filled from GPS above, or type: City, locality, state")
            col1,col2=st.columns(2)
            ...
            if submitted and sighting_desc and sighting_location:
                if not selected_case: st.error("Please search and select a case above first.")
                else:
                    result=analyse_sighting(selected_case,sighting_desc,sighting_location)
                    score=result.get("confidence_score",0); level=result.get("match_level","LOW")
                    st.markdown("---")
                    col1,col2,col3=st.columns(3)
                    col1.metric("Confidence Score",f"{score}%"); col2.metric("Match Level",level); col3.metric("Case",selected_case.get("case_id","N/A"))
                    st.progress(min(score,100)/100)
                    st.markdown(f"**Reasoning:** {result.get('reasoning','N/A')}")
                    for f in result.get("matching_features",[]): st.markdown(f"- ✅ {f}")
                    if result.get("trigger_consent_wall") or score>=60:
                        st.markdown("""<div class="consent-wall">
  <div style="font-size:3rem">🔒</div>
  <h2 style="color:#e94560;font-size:1.8rem;margin:0.5rem 0">CONSENT VERIFICATION REQUIRED</h2>
  <p style="color:#ffffff;font-size:1.1rem;margin:1rem 0">High-confidence match detected.<br>
  <strong>Both parties must independently verify Aadhaar and give consent before any contact information is shared.</strong></p>
  <p style="color:#a0a0b0;font-size:0.9rem">This protects the privacy, safety, and autonomy of all individuals.<br>Consent cannot be assumed. Reunion is never forced.</p>
</div>""",unsafe_allow_html=True)
                        ...
                        if fam_ok and ngo_ok:
                            ...
                            st.markdown(f'<div style="background:#0f2a0f;border:2px solid #4caf50;border-radius:10px;padding:1.5rem;margin:1rem 0"><h3 style="color:#4caf50;margin:0 0 8px">✅ Reunion Protocol Activated</h3><div style="color:#a0c0a0;font-size:13px;line-height:1.8">Case: <strong>{selected_case.get("name")}</strong> ({selected_case.get("case_id")})<br>Match confidence: <strong>{score}%</strong><br>NGO mediator assigned<br>Contact details shared through secure NGO channel only</div></div>',unsafe_allow_html=True)
                            st.balloons()
                        else:
                    ...
                    else: st.warning(f"Low confidence ({score}%). Sighting recorded but threshold not met for consent wall.")
...
# ── AGE PROGRESSION ────────────────────────────────────────────────────────────
    ...
    if cases:
        ...
        years=2026-case.get("year_missing",2000); age_then=case.get("age_when_missing",0); age_now=age_then+years
        st.markdown(f'<div class="timeline-bar"><div style="text-align:center;min-width:80px"><div style="color:#e94560;font-weight:600">{case.get("year_missing")}</div><div style="color:#a0a0b0;font-size:12px">Age {age_then}</div></div><div style="flex:1;height:4px;background:linear-gradient(90deg,#e94560,#ff9800,#4caf50);border-radius:2px"></div><div style="text-align:center;min-width:80px"><div style="color:#4caf50;font-weight:600">2026</div><div style="color:#a0a0b0;font-size:12px">Age ~{age_now}</div></div><div style="color:#a0a0b0;font-size:13px;margin-left:12px">{years} years</div></div>',unsafe_allow_html=True)
        col1,col2=st.columns(2)
        with col1:
            ...
            if case.get("distinguishing_marks"): st.markdown(f"- 🔍 **Marks:** `{case.get('distinguishing_marks')}`")
        with col2:
        ...
        if st.button("🧬 Run AI Age Progression",type="primary"):
            ...
            st.markdown(f'<div style="background:#1a1a2e;border:1px solid #4caf50;border-radius:10px;padding:1.2rem;margin:1rem 0"><p style="color:#e0e0e0;font-size:15px;line-height:1.7;margin:0">{result.get("estimated_current_appearance","N/A")}</p></div>',unsafe_allow_html=True)
            st.markdown("**🔍 Permanent identifiers (will never change):**")
            for f in result.get("key_identifying_features",[]): st.markdown(f"- 🔍 {f}")
            if result.get("likely_changes"):
                st.markdown("**Changes over time:**")
                for ch in result.get("likely_changes",[]): st.markdown(f"- {ch}")
...
# ── RISK ASSESSMENT ────────────────────────────────────────────────────────────
    ...
    if cases:
        ...
        if st.button("⚠️ Run Risk Assessment",type="primary"):
            ...
            st.markdown("---")
            col1,col2,col3=st.columns(3)
            col1.metric("Urgency",f"{emoji} {urgency}"); col2.metric("Risk Score",f"{result.get('risk_score',0)}/100"); col3.metric("Recommended Tier",f"Tier {result.get('recommended_tier',2)}")
            st.progress(result.get('risk_score',0)/100)
            st.markdown(f"**Primary Risk:** {result.get('primary_risk','N/A')}")
            if result.get("special_flags"): st.markdown("**Flags:** "+" ".join([f"`{f}`" for f in result.get("special_flags",[])]))
            st.markdown("**Next Steps:**")
            for i,step in enumerate(result.get("next_steps",[]),1): st.markdown(f"{i}. {step}")
            if result.get("consent_required"): st.warning("🔒 Consent wall required for this case")
...
# ── UNIDENTIFIED PERSON ────────────────────────────────────────────────────────
    ...
    with st.form("unidentified_form"):
        col1,col2=st.columns(2)
        with col1:
            description=st.text_area("Physical description *",height=100,placeholder="Age estimate, height, complexion, hair, scars, tattoos, clothing...")
        ...
        with col2:
            ...
            behaviour=st.text_area("Behaviour notes",height=100,placeholder="Disoriented, memory loss, mentions certain names...")
        ...
        if submitted and description and location:
            ...
            with col2: st.markdown(f"**Summary:** {result.get('identity_summary','N/A')}")
            st.markdown("**Search Strategy:**")
            for i,step in enumerate(result.get("search_strategy",[]),1): st.markdown(f"{i}. {step}")
            tags=result.get("match_tags",[])
            if tags:
                st.markdown("**Elastic Search Tags:**"); st.code(", ".join(tags))
                if st.button("🔍 Cross-reference with Elastic cases"):
                    ...
                    if matches:
                        ...
                        for m in matches: st.markdown(f"- **{m.get('name')}** ({m.get('case_id')}) — {m.get('status','')}")
                    else: st.info("No matches in current database.")

# ── AGENT CHAT ─────────────────────────────────────────────────────────────────
elif page=="💬 Agent Chat":
    st.markdown("## 💬 REUNION Agent")
    if "messages" not in st.session_state:
        st.session_state.messages=[{"role":"assistant","content":"Namaste 🙏 I am the REUNION agent.\n\nI help reunite missing persons with their families across India.\n\nI can:\n- 🔍 Search for a missing person case\n- 🧬 Explain age progression results\n- 🔒 Explain the consent wall process\n- ⚠️ Answer questions about safety and access tiers\n\nHow can I help you today?"}]
    st.markdown("**💡 Suggested questions:**")
    suggestions=["Search for Lakshmi Bai missing from Kolkata","How does the consent wall work?","What is the 4-tier access system?","Find unidentified cases in Mumbai"]
    cols=st.columns(2)
    for i,s in enumerate(suggestions):
        with cols[i%2]:
            if st.button(s,key=f"sug_{i}",use_container_width=True):
                st.session_state.messages.append({"role":"user","content":s}); st.rerun()
    st.markdown("---")
    ...
    if prompt:=st.chat_input("Ask the REUNION agent..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                reply=ask_ai_chat(st.session_state.messages,context)
            st.markdown(reply)
